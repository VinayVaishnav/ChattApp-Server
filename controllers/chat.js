...
const getMyChats=TryCatch(async (req,res,next)=>{
    ...
    const transformedChats=chats.map(({_id,name,members,groupChat})=>{
        ...
        return {
            ...
            .map(({avatar})=>avatar.url): [otherMember.avatar.url],
...
const removeMember =TryCatch(async (req,res,next)=>{
    ...
    await chat.save();
    emitEvent(req,ALERT,chat.members, `${usersThatWillBeRemoved.name} has been removed`)
...
const sendAttachment=TryCatch(async(req,res,next)=>{
    ...
    if(files.length >5) return next (new ErrorHandler("You can not Upload more than 5 files",400));
    ...
    if(files.length<1) return next(new ErrorHandler("please send attachment",404));
...
const renameGroup=TryCatch(async(req,res,next)=>{
    ...
    if(!chat.groupChat) return next (new ErrorHandler("Thisis not a group chat",404));

    if(chat.creator.toString()!== req.user.toString())
        return next(new ErrorHandler("You are not allowed to renmae the group",403));
...
const deleteChat=TryCatch(async(req,res,next)=>{
    ...
    console.log("messageswith attach");
...
const getMessage =TryCatch( async (req, res, next)=>{
    ...
    if(!chat.members.includes(req.user.toString()))
        return next(
            new ErrorHandler("You are not allowed to accces this chat",403)
        );
...
export {
    addMembers, deleteChat, getChatDetails, getMessage, getMyChats,
    getMyGroups, leaveGroup, newGroup, removeMember, renameGroup, sendAttachment,emitEvent
};
